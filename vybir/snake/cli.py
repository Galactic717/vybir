"""Live terminal Snake where every move is a real Laya decision. Also: `export`."""
...
class Keyboard:
    """Non-blocking keys: msvcrt on Windows, cbreak termios elsewhere.

    Arrow keys are reported as the lowercased ANSI sequences the loop matches.
    """
    ...
    def read(self):
        if os.name == "nt":
            ...
            while msvcrt.kbhit():
                key = msvcrt.getwch()
                if key in ("\x00", "\xe0"):  # arrow/function prefix
                    key = {"H": "\x1b[a", "P": "\x1b[b"}.get(msvcrt.getwch(), "")
                keys += key
            return keys
        import select
...
def play(argv=None):
    parser = argparse.ArgumentParser(
        prog="vybir snake",
        description=__doc__,
        epilog="Also: vybir snake export --help",
    )
    parser.add_argument("--model", help="Local model directory or an already cached Hub ID")
    parser.add_argument("--subfolder", help="e.g. multilingual (smaller, faster checkpoint)")
    parser.add_argument("--backend", choices=("torch", "ort"), default="torch")
    parser.add_argument("--ort-path", help="ONNX file from `vybir export-onnx` (backend=ort)")
    parser.add_argument("--device", choices=("auto", "cuda", "cpu"), default="auto")
    parser.add_argument("--prompt", choices=("compact", "detailed"), default="compact")
    parser.add_argument(
        "--optimize",
        action="store_true",
        help="Enable 16-token buckets and bounded prefix reuse",
    )
    ...
    print("Loading cached weights; no network requests...", file=sys.stderr)
    try:
        policy = LayaPolicy(
            args.model, subfolder=args.subfolder, guarded=not args.unassisted,
            prompt=args.prompt, optimize=args.optimize, backend=args.backend,
            ort_path=args.ort_path, device=args.device,
        )
