# Derived from Laya (Apache-2.0); see NOTICE. Modified for laya-mlx and vybir.
...
def embed_fn_from_agent(
    ...
    tok = agent.tok
    if getattr(agent, "backend", "torch") != "torch" or getattr(agent, "model", None) is None:
        raise RuntimeError("embed_fn_from_agent needs the torch backend (ORT exposes no encoder)")
    ...
    def embed_fn(texts: Sequence[str]) -> np.ndarray:
        ...
        for start in range(0, len(rows), batch_size):
            ...
            with torch.inference_mode():
                ...
                hidden_states = encoder(iid, am).float()
                mask = am.float().unsqueeze(-1)
                pooled = (hidden_states * mask).sum(1) / mask.sum(1).clamp_min(1.0)
