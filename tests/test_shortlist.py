...
decision model is never constructed. The torch ``embed_fn_from_agent`` is exercised against
...
from vybir.shortlist import embed_fn_from_agent, predict_shortlist, shortlist_choice
...
def test_exports():
    assert vybir.shortlist_choice is shortlist_choice
    assert vybir.predict_shortlist is predict_shortlist
    assert vybir.embed_fn_from_agent is embed_fn_from_agent
    for name in ("shortlist_choice", "predict_shortlist", "embed_fn_from_agent"):
        assert name in vybir.__all__
